# Исключения  (exceptions) - еще один тип данных в питон.
#Исключения необходимы для того, чтобы сообщать программисту об
#if ошибках. Самый простой тип исключения - деление на ноль
#Как правило, интерпретатор выдаст нам ошибку ZeroDivizionError: division by zero
#Для обработки исключения используется конструкция try/except
#В блоке try мы выполняем инструкцию, которая может создать искючение,
